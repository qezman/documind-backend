...
import { insertChat, getChatHistory as getRepoChatHistory } from './chat.repository.js';
...
export const askDocument = async ({
  ...
  if (doc.status !== 'ready') throw new AppError('Document is still processing', 409);
  ...
  if (chunks.length === 0) {
    const text = "I couldn't find relevant information in this document to answer your question.";
    onToken(text);
