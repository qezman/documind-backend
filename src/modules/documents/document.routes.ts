...
export const documentRoutes = async (app: FastifyInstance): Promise<void> => {
  ...
  // Step 1 of upload: get a presigned PUT URL
  app.post('/upload', async (req, reply) => {
    const { filename, mimeType } = req.body as { filename: string; mimeType: string };
    if (!filename || !mimeType) {
      return reply.status(400).send({ message: 'filename and mimeType are required' });
    }
    const result = await uploadDocument(req.user.id, filename, mimeType);
    return reply.status(201).send(result);
  });

  // Step 2: trigger text extraction + embedding pipeline
